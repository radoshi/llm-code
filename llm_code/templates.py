...
class Template(BaseModel):
    ...
    def save(self, filename: str | Path = "") -> None:
        ...
        filename.write_text(self.model_dump_json(), encoding="utf-8")
    ...
    @classmethod
    def parse_json(cls, filename: Path | str) -> Template:
...
class TemplateLibrary(UserDict):
    ...
    @classmethod
    def from_file_or_directory(cls, filename_or_dir: str | Path) -> TemplateLibrary:
        ...
        if path.is_dir():
            for filename in path.glob("**/*.json"):
                library.add(Template.parse_json(filename))
            ...
            if path.suffix == ".toml":
                ...
                library.add(Template.parse_json(path))
        return library
